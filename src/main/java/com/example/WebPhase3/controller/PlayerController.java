...
public class PlayerController {
    ...
    @GetMapping("/{username}")
    public Player getPlayerByUsername(@PathVariable String username, @RequestHeader("Authorization") String token) {
        if (!jwtUtil.validateToken(token.substring(7))) {
            throw new RuntimeException("Invalid JWT Token");
        }
    ...
    public ResponseEntity<?> getAnsweredQuestionsWithTarrahInfo(@PathVariable String username, @RequestHeader("Authorization") String token) {
        if (!jwtUtil.validateToken(token.substring(7))) {
    ...
    public String updateFollowing(@PathVariable String username, @RequestBody Map<String, String> requestBody, @RequestHeader("Authorization") String token) {
        if (!jwtUtil.validateToken(token.substring(7))) {
            throw new RuntimeException("Invalid JWT Token");
        }
